...
/**
 * Verifies AIpaca-Ed25519 Authorization headers.
 *
 * Header format:
 *   Authorization: AIpaca-Ed25519 <base64-raw-pubkey> <base64-signature> <unix-timestamp>
 *
 * Signed message: "AIpaca-Ed25519:<unixTimestampSeconds>"
 */
object Ed25519Verifier {
    ...
    fun verify(authHeader: String): Result {
        ...
        if (parts.size != 4 || parts[0] != "AIpaca-Ed25519") {
            return Result.Invalid("Malformed Authorization header; expected: AIpaca-Ed25519 <pubkey> <sig> <timestamp>")
        }
        ...
        // Verify signature over "AIpaca-Ed25519:<timestamp>"
        val message = "AIpaca-Ed25519:$tsStr".toByteArray(Charsets.UTF_8)
        val valid = try {
